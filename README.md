# Olá, eu sou Estêvão 👋

```txt
Backend Developer • Java • Spring Boot • Engenharia + Tecnologia
```

💻 Desenvolvedor focado em Backend com Java  
🏗️ Engenheiro Civil migrando para Tecnologia  
🚀 Apaixonado por sistemas, automações e soluções inteligentes  
📚 Estudando constantemente arquitetura, performance e boas práticas  

---

## 🐍 Snake Animation

<p align="center">
  <img src="https://raw.githubusercontent.com/Estevao871/Estevao871/output/github-contribution-grid-snake.svg" />
</p>

# 🚀 Tecnologias & Ferramentas

## Backend
<p>
  <img src="https://img.shields.io/badge/Java-ED8B00?style=for-the-badge&logo=openjdk&logoColor=white"/>
  <img src="https://img.shields.io/badge/Spring_Boot-6DB33F?style=for-the-badge&logo=spring&logoColor=white"/>
  <img src="https://img.shields.io/badge/Spring_Security-6DB33F?style=for-the-badge&logo=springsecurity&logoColor=white"/>
  <img src="https://img.shields.io/badge/JPA-Hibernate-blue?style=for-the-badge"/>
</p>

## Frontend
<p>
  <img src="https://img.shields.io/badge/Angular-DD0031?style=for-the-badge&logo=angular&logoColor=white"/>
  <img src="https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white"/>
  <img src="https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge&logo=css3&logoColor=white"/>
  <img src="https://img.shields.io/badge/JavaScript-black?style=for-the-badge&logo=javascript"/>
</p>

## Banco de Dados
<p>
  <img src="https://img.shields.io/badge/MySQL-4479A1?style=for-the-badge&logo=mysql&logoColor=white"/>
  <img src="https://img.shields.io/badge/PostgreSQL-316192?style=for-the-badge&logo=postgresql&logoColor=white"/>
</p>

## Ferramentas
<p>
  <img src="https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white"/>
  <img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github"/>
  <img src="https://img.shields.io/badge/IntelliJ_IDEA-black?style=for-the-badge&logo=intellijidea"/>
  <img src="https://img.shields.io/badge/VS_Code-007ACC?style=for-the-badge&logo=visualstudiocode"/>
</p>

---

# 🧠 Sobre Mim

Minha trajetória começou na engenharia civil, mas encontrei na tecnologia uma forma de criar soluções práticas, automatizar processos e desenvolver sistemas úteis para pessoas e negócios.

Hoje meu foco principal está em:

- Desenvolvimento Backend com Java
- APIs REST
- Arquitetura de Sistemas
- Spring Boot
- Integrações
- Performance e boas práticas
- Projetos pessoais

---

# 📌 Projetos em Destaque

### 🏥 Sistema para Clínica de Fisioterapia
Projeto voltado para gestão clínica, organização de pacientes e evolução de atendimentos.

### ⚙️ API REST com Spring Boot
Estrutura backend utilizando boas práticas, autenticação e arquitetura organizada.

### 📊 Dashboard Administrativo
Painel administrativo utilizando Angular integrado com APIs Java.

### 🧩 Estudos & Laboratórios
Repositório dedicado a testes, conceitos e aprendizado contínuo em tecnologia.

---

# 📚 Atualmente Estudando

- Microsserviços
- Docker
- Kubernetes
- Clean Architecture
- Segurança com Spring Security
- Performance Backend

---

# 📈 Estatísticas GitHub

<p align="center">
  <img height="180em" src="https://github-readme-stats.vercel.app/api?username=SEU_USUARIO&show_icons=true&theme=tokyonight&include_all_commits=true&count_private=true"/>

  <img height="180em" src="https://github-readme-stats.vercel.app/api/top-langs/?username=SEU_USUARIO&layout=compact&langs_count=7&theme=tokyonight"/>
</p>

---

# 🔥 Sequência de Contribuições

<p align="center">
  <img src="https://streak-stats.demolab.com?user=SEU_USUARIO&theme=tokyonight"/>
</p>

---

# 🎯 Objetivos

- Evoluir como Engenheiro de Software
- Construir projetos reais e escaláveis
- Aprofundar conhecimentos em arquitetura backend
- Integrar tecnologia com soluções do mundo real

---

# 📫 Contato

<p>
  <a href="https://linkedin.com/in/SEU_LINKEDIN">
    <img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin"/>
  </a>
</p>

---

<p align="center">
  <i>"Transformando ideias em soluções através da tecnologia."</i>
</p>
